    ...
    with open(i_file, 'r') as ifile:
        ...
        while True:
            ...
            try:
                ...
                try:
                    curs.executemany('insert into {} ({}) values({})'.format(motif_table, ', '.join(motif_cols_names),
    ...
    for i_file in scored_motifs_overlapping_tracks_files_in:  # [f for f in glob.glob('{}/*{}*'.format(dir_to_import, keyword_to_check))]
        if run_in_parallel_param and len(scored_motifs_overlapping_tracks_files_in) > 1:
            ...
            p.apply_async(insert_from_file,
                          args=[motif_cols_names, i_file, 100000, db_name, db_user_name, db_host_name, motif_table,
                                thread_num])
        else:
...
def generate_PFM_table(PFMs_dict, PFM_table_name,
    ...
    '''
    rows_to_insert = []
    for motif in list(PFMs_dict.keys()):
        for pos in range(0, len(PFMs_dict[motif])):
            PFMs_dict[motif][0]
            for allele in list(PFMs_dict[motif][pos].keys()):
    ...
    conn.commit()
    curs.executemany(
        'insert into {} ({}) values({})'.format(PFM_table_name, ', '.join(cols_names), ', '.join(value_marks)),
        rows_to_insert)
...
def create_motifs_table(db_name, db_user_name, db_host_name, motifs_table, motif_cols, new_table_name):
    ...
    print(('create table if not exists {0} as (select {1} from {2});'.format(new_table_name, ','.join(motif_cols),
                                                                             motifs_table)))
    ...
    curs.execute('create table if not exists {0} as (select {1} from {2});'.format(new_table_name.split(" ")[0],
                                                                                   ','.join(motif_cols), motifs_table))
    curs.execute('create index if not exists {0}mid on {1} using btree(mid);'.format(new_table_name, new_table_name))
    curs.execute(
        'create index if not exists {0}posrange on {1} using gist(posrange);'.format(new_table_name, new_table_name))
    curs.execute(
        'create index if not exists {0}tfname on {1} using btree(name);'.format(new_table_name, new_table_name))
...
def create_motifs_table_from_file(db_name, db_user_name, db_host_name, scored_motifs_overlapping_tracks_files,
                                  motif_cols, motif_cols_names, new_table_name, run_in_parallel_param,
                                  number_processes_to_run_in_parallel):
    # create database
    conn = DBUtilities.open_connection(db_name, db_user_name, db_host_name)
    curs = conn.cursor()
...
def split_motifs_parallel(db_name, db_user_name, db_host_name, motifs_table, chr, motif_cols):
    ...
    curs.execute('drop table if exists {0};'.format(new_table_name))
    print(('create table if not exists {0} as (select {1} from {2} where chr={3});'.format(new_table_name,
                                                                                           ','.join(motif_cols),
                                                                                           motifs_table, chr)))
    curs.execute('create table if not exists {0} as (select {1} from {2} where chr={3});'.format(new_table_name,
                                                                                                 ','.join(motif_cols),
                                                                                                 motifs_table, chr))
    curs.execute('create index if not exists {0}mid on {1} using btree(mid);'.format("chr" + str(chr), new_table_name))
    curs.execute(
        'create index if not exists {0}posrange on {1} using gist(posrange);'.format("chr" + str(chr), new_table_name))
    conn.commit()
    curs.close()
    conn.close()
    return
...
def motif_names_table(db_name, db_user_name, db_host_name,
    ...
    print(
        ('create table if not exists {0} as (select distinct(name) from {1});'.format(motif_names_table, motifs_table)))
    curs.execute(
        'create table if not exists {0} as (select distinct(name) from {1});'.format(motif_names_table, motifs_table))
    conn.commit()
    curs.close()
