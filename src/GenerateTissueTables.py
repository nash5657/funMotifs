        ...
        for line in lines:
            ...
            if key_value not in list(tissue_cell_assays.keys()):
                ...
                for assay_name in assay_names:
                    assay_name = '_'.join(
                        assay_name.strip().replace('(', '').replace(')', '').replace('-', '__').split())
                ...
                if cell in list(cell_assays.keys()):
                    ...
                    if ':' in s:
                        ...
                        if assay in cell_assays[cell]:
                            assay_updated_name = '_'.join(
                                assay.replace('(', '').replace(')', '').replace('-', '__').replace('.', '').split())
                            try:
                                tissue_cell_assays[key_value][assay_updated_name].append(
                                    (cell_updated_name + '___' + assay_updated_name).lower())
                            except KeyError:
                                tissue_cell_assays[key_value][assay_updated_name] = [
                                    (cell_updated_name + '___' + assay_updated_name).lower()]
        ...
        try:
            if float(value) > 0.0:
                if assay.lower() != "numothertfbinding":
                    score = feature_weights_dict[
                        assay.upper()]  # where the assay name is present in the weights conf file
                else:
                    v = float(value)
                    if v > 3.0:
                        v = 3.0
...
def insert_into_tissues(selected_rows, tissue_cell_assays, tissue_cell_allassays, assay_names,
        ...
        for assay in assay_names:
            ...
            if len(tissues_with_NaN_values) > 0:
                if len(tissues_with_values) >= 4:
                    value = 'NaN'
                    try:  # FIX: For peaks (ChIP-seq & DNase-seq) don't take the average, rather check if there are
                        # more than 4 samples that have a peak then set it to 1 otherwise set it to zero,
                        # currently if only one cell has a peak with value x and all other cells have no peak for a
                        # motif the imputed value would still be larger than zero.
    ...
    fscores_per_tissues_dataText = ','.join(
        '(' + curs_for_insertion.mogrify(s_chars_for_fscores, row).decode("utf-8") + ')' for row in
        t_tissues_fscores_values)
    curs_for_insertion.execute(
        'insert into {table_name} ({field_names}) values {values}'.format(table_name=tissues_fscores_table,
                                                                          field_names=', '.join(
...
def insert_into_tissues_from_file(scored_motifs_overlapping_tracks_files_tissue, header_scored_lower, mid_value,
                                  tissue_cell_assays_file, tissue_cell_allassays_file, assay_names,
                                  cols_to_write_to,
                                  cols_to_write_to_allassays, thread_num,
    ...
    for tissue in sorted(tissues_values.keys()):
        ...
        try:
            curs_for_insertion.execute(
                'insert into {table_name} ({field_names}) values ({values})'.format(table_name=tissue,
                                                                                    field_names=field_names,
                                                                                    values=dataText))
            tissues_names_for_fscores = ['mid']
            tissues_names_for_fscores.extend(sorted(tissue_cell_allassays.keys()))
            ...
            t_tissues_fscores_values = tuple(fscores_per_tissues_allrows)
            fscores_per_tissues_dataText = ','.join(
                '(' + curs_for_insertion.mogrify(s_chars_for_fscores, row) + ')' for row in t_tissues_fscores_values)
            curs_for_insertion.execute(
                'insert into {table_name} ({field_names}) values {values}'.format(table_name=tissues_fscores_table,
                                                                                  field_names=', '.join(
                                                                                      tissues_names_for_fscores),
...
def insert_into_tissues_per_tissue(scored_motifs_overlapping_tracks_files, tissue_cell_assays, tissue_cell_allassays,
                                   assay_names,
                                   cols_to_write_to,
                                   cols_to_write_to_allassays, thread_num,
                                   feature_weights_dict,
...
def populate_tissue_values(tissue_cell_assays, tissue_cell_allassays, assay_names, col_list, table_from,
    ...
    if run_in_parallel_param and len(scored_motifs_overlapping_tracks_files) > 1:
        ...
        while i < num_cores:
            p.apply_async(insert_into_tissues,
                          args=(selected_rows, tissue_cell_assays, tissue_cell_allassays, assay_names,
...
def populate_tissue_values_from_scored_files(tissue_cell_assays, tissue_cell_allassays, assay_names, col_list,
    ...
    if run_in_parallel_param and len(scored_motifs_overlapping_tracks_files) > 1:
        ...
        for file_in in scored_motifs_overlapping_tracks_files:
            ...
            comm_divide_files = "cat {} | tail -n +2 | split -l {} - {}".format(file_in, str(number_of_rows_to_load),
                                                                                file_in + "_part")
            ...
            for part_file in part_file_list:
                ...
                res = p.apply_async(insert_into_tissues_from_file,
                                    args=[part_file, header_scored_lower, mid_value, tissue_cell_assays_file,
                                          tissue_cell_allassays_file, list(assay_names),
        ...
        while selected_rows:
            ...
            t_for_fetch = time.time()
            selected_rows = []
            selected_rows_df = pd.read_csv(file_in, skiprows=list(range(n_lines, n_lines_end)),
                                           nrows=number_of_rows_to_load, sep='\t', dtype=str)
            selected_rows_df_order = selected_rows_df.reindex(columns=col_list[1::])
            # to list of tuples
            records = selected_rows_df_order.to_records(index=False)
            selected_rows = list(records)
...
def populate_tissue_values_from_scored_files_per_tissue(tissue_cell_assays, tissue_cell_allassays, assay_names,
                                                        col_list,
                                                        motif_cols_names,
                                                        run_in_parallel_param,
                                                        number_processes_to_run_in_parallel,
    ...
    if run_in_parallel_param and len(scored_motifs_overlapping_tracks_files) > 1:
        ...
        for file_in in scored_motifs_overlapping_tracks_files:
            ...
            while i < num_cores:
                ...
                comm_cut_tissue = (
                    'cut -f{} {} > {}'.format(','.join([str(id) for id in tissue_cell_assays_id]), file_in,
                                              scored_motifs_overlapping_tracks_files_tissue))
                print(comm_cut_tissue)
                os.system(comm_cut_tissue)

                p.apply_async(insert_into_tissues_per_tissue, args=(
                scored_motifs_overlapping_tracks_files_tissue, tissue_cell_assays, tissue_cell_allassays, assay_names,
                cols_to_write_to, cols_to_write_to_allassays, thread_num, feature_weights_dict,
                db_name, db_user_name, db_host_name, tissues_fscores_table))
                # num_rows -=len(selected_rows)
                print(file_in)
...
def generate_tissue_tables(db_name,
    ...
    for tissue_table in sorted(tissue_cols.keys()):
        DBUtilities.create_index(db_name, db_user_name, db_host_name,
                                 cell_table=tissue_table, index_name='index' + tissue_table + 'mid',
                                 index_method='btree', index_cols='mid')
    DBUtilities.create_index(db_name, db_user_name, db_host_name,
                             cell_table=tissues_fscores_table, index_name='index' + tissues_fscores_table + 'mid',
                             index_method='btree', index_cols='mid')
